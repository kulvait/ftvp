...
init_memory(struct memory_cuda* mem, struct res_cuda* res, int sx, int sy, int sz, double* u)
{
    ...
    double* dev_xiobar; // gpu versions
    ...
    gpuErrchk(
        cudaMemcpy(dev_xie, xie, Ke * Le * Me * factor * sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(
        cudaMemcpy(dev_xio, xio, Ko * Lo * Mo * factor * sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(dev_xiobar, xiobar, Ko * Lo * Mo * factor * sizeof(double),
