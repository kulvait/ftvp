...
init_memory(struct memory_cuda* mem, struct res_cuda* res, int sx, int sy, int sz, double* u)
{
    ...
    gpuErrchk(cudaMemset(dev_xie, 0, Ke * Le * Me * factor * sizeof(double)));
    gpuErrchk(cudaMemset(dev_xio, 0, Ko * Lo * Mo * factor * sizeof(double)));
    // Additional sanitization, not in original code
    gpuErrchk(cudaMemset(dev_xiobar, 0, Ko * Lo * Mo * factor * sizeof(double)));
    gpuErrchk(cudaMemset(dev_gle, 0, Ke * Le * Me * sizeof(double)));
    gpuErrchk(cudaMemset(dev_glo, 0, Ko * Lo * Mo * sizeof(double)));
