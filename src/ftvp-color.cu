                               ...
                               double gap_factor)
{
    ...
    do
    {
        ...
        t = .5
            * ((1 - q * t * t)
               + sqrt((1 - q * t * t) * (1 - q * t * t) + 4 * (1 + gammap) / (1 + gamma) * t * t));
        theta = (tt - 1) / t * (1 - (t - 1) * gammap);
        if(theta > 0)
        {
            over_relax_eps_2d<<<blocks_odd, threads>>>(sx, sc, mem->dev_xio, mem->dev_xiobar,
                                                       mem->dev_u, theta, Lo, Ko);
        ...
        // Minimization
        opt_eps_split<<<blocks_even, threads>>>(sx, sc, mem->dev_xie, mem->dev_u, epsilon, w, ws,
                                                we2, Le, Ke, steps, 1);
        opt_eps_split<<<blocks_odd, threads>>>(sx, sc, mem->dev_xiobar, mem->dev_u, epsilon, w, ws,
                                               we2, Lo, Ko, steps, 0);

        // Gap
        if(it % gapiter == 0)
        {
            gap_arr_eps_2d<<<blocks_odd, threads>>>(sx, sc, mem->dev_glo, mem->dev_xio, mem->dev_u,
            ...
            gap_arr_eps_2d<<<blocks_even, threads>>>(sx, sc, mem->dev_gle, mem->dev_xie, mem->dev_u,
