...
init_memory(struct memory_cuda* mem, struct res_cuda* res, int sx, int sy, int sc, double* u)
{
    ...
    gpuErrchk(
        cudaMemcpy(dev_xie, xie, Ke * Le * sc * factor * sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(
        cudaMemcpy(dev_xio, xio, Ko * Lo * sc * factor * sizeof(double), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(dev_xiobar, xiobar, Ko * Lo * sc * factor * sizeof(double),
